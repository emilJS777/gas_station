...
# CREATE DEVICE
def create_device(key: str, name: str, description: str, client_id: int, cash_box_id: int) -> dict:
    # GET CLIENT AND CASH BOX IF NOT FOUND RETURN NOT FOUND
    if not client_service_db.get_by_id(client_id=client_id) or not CashBoxServiceDb.get_by_id(cash_box_id):
...
# UPDATE DEVICE
def update_device(device_id: int, key: str, name: str, description: str, parent_key: str, cash_box_id: int) -> dict:
