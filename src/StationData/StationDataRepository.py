...
def get_by_id(station_data_id: int) -> StationData:
    if g.cashier:
        station_data: StationData = StationData.query.filter_by(id=station_data_id, cash_box_id=g.cash_box_id,
                                                                cashier_id=g.user_id, client_id=g.client_id).first()
    elif g.cash_box_id:
        station_data: StationData = StationData.query.filter_by(id=station_data_id,
...
def get_all_ids_by_cash_box_id(cash_box_id: int, date) -> List[int]:
    if g.cashier:
        ...
        stations_data: List[StationData] = StationData.query.filter_by(client_id=g.client_id, cash_box_id=cash_box_id, creation_date=date).all()
